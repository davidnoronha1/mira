...
std::vector<Stage> loadStagesFromXML(const std::string &filepath) {
  ...
  for (tinyxml2::XMLElement *stageElem = root->FirstChildElement("stage");
    ...
    if (stageElem->QueryFloatAttribute("duration", &stage.duration) !=
...
int main(int argc, char **argv) {
  ...
  std::filesystem::path share_path(share_dir);

  // Go up from install/share/your_package to workspace root
  std::filesystem::path workspace =
      share_path.parent_path().parent_path().parent_path().parent_path();

  RCLCPP_INFO(node->get_logger(), "Workspace path: %s", workspace.string().c_str());
  ...
  std::string xml_path = workspace / "video.xml";
  RCLCPP_INFO(node->get_logger(), "Loading stages from: %s",
              xml_path.c_str());
