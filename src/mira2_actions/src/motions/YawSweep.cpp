...
// YawSweep: perform an oscillating yaw sweep around a center heading
// Uses the shared ROSState instance for telemetry and publishing commands.
class YawSweep : public BT::SyncActionNode
{
    PID_Controller yaw_pid;
    ROSState* ros_state_;

    // sweep parameters
    double center_heading_ = 0.0;
    double amplitude_ = 30.0; // degrees from center to each extreme
    int cycles_ = 1; // number of back-and-forth cycles to perform
    double tolerance_ = 5.0; // degrees tolerance for reaching a target
    double dwell_time_ = 1.0; // seconds to hold at each extreme
    double timeout_ = 60.0; // maximum allowed time

    // runtime state
    std::vector<double> targets_; // sequence of headings to visit
    size_t current_target_idx_ = 0;
    int completed_cycles_ = 0; // counts completed back-and-forths
...
public:
    YawSweep(const std::string& name, const BT::NodeConfiguration& config, ROSState* ros_state)
        : BT::SyncActionNode(name, config), yaw_pid("yaw", ros_state->node), ros_state_(ros_state)
    {
    }
    ...
    BT::NodeStatus tick() override
    {
        ...
        float pid_yaw = yaw_pid.pid_control(static_cast<float>(yaw_error), static_cast<double>(elapsed), false);
