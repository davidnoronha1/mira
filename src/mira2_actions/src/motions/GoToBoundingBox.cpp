...
ApproachBB::ApproachBB(const std::string& name, 
    ...
    yaw_pid("yaw_lock_gtbb",ros_state->node),
    forward_pwm_(1550.0),
    frame_width_(1.0),
    frame_height_(1.0),
    ...
    bb_found_(false),
    bb_x_center_norm_(0.5),
    bb_y_center_norm_(0.8),
    bb_area_norm_(0.0),
    depth_setpoint_base_(0.0),
    depth_visual_gain_(10.0),
    locked_heading_(0.0)
{
...
BT::PortsList ApproachBB::providedPorts()
{
    return {
        ...
        // Frame resolution — set these if your vision node gives raw pixels.
            ...
            BT::InputPort<std::string>("flight_mode", "MANUAL",
                "FC mode: ALT_HOLD (auto depth hold) or STABILIZE (manual thrust)"),

            // Depth PID — adjusts thrust to hold depth, setpoint shifted by visual Y error
            BT::InputPort<double>("depth_setpoint",       0.0,    "Baseline pressure setpoint (same units as external_pressure)"),
            BT::InputPort<double>("depth_visual_gain",    10.0,   "Gain scaling how much Y-error shifts the depth setpoint"),
            BT::InputPort<double>("depth_pid_kp",         0.05,   "Depth PID proportional gain"),
            ...
            BT::InputPort<double>("depth_pid_base_offset",1580.0, "Depth PID base offset (neutral thrust PWM)"),

            // Yaw lock PID — holds initial heading against rotational disturbances
            BT::InputPort<double>("yaw_pid_kp",          1.5,    "Yaw lock PID proportional gain"),
            ...
            BT::InputPort<double>("yaw_pid_base_offset", 1500.0, "Yaw lock PID base offset (neutral PWM)"),
...
BT::NodeStatus ApproachBB::onRunning()
{
    ...
    // ── 5. Compute horizontal centering error ─────────────────────────────
        ...
        double adjusted_depth_setpoint = depth_setpoint_base_ + depth_visual_gain_ * y_error;
        double depth_error = ros_state_->telemetry.external_pressure - adjusted_depth_setpoint;
