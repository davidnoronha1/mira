...
Yolo11OpenVinoModel::Yolo11OpenVinoModel(rclcpp::Logger logger,
                                         const std::string &device, int height,
                                         int width)
    : ImageProcessor(logger, device, height, width), input_height_(height),
      input_width_(width) {
  package_share_dir =
  ...
  RCLCPP_INFO(logger_, "Input image height: %d, width: %d", input_height_,
              input_width_);

  model_path =
      package_share_dir + "/models/yolo11n_openvino_model_int8/yolo11n.xml";
  ...
  try {
    ...
    throw std::runtime_error(std::string("Failed to load YOLO model: ") +
                             e.what());
  }
}
...
Yolo11OpenVinoModel::processImage(const cv::Mat &image) {
  ...
  req.start_async();
  req.set_callback([&](std::exception_ptr exptr) {
    if (exptr != nullptr) {
...
void Yolo11OpenVinoModel::initializeClassNames() {
  ...
  if (!file.is_open()) {
    RCLCPP_ERROR(logger_, "Failed to open class names file: %s",
  ...
  RCLCPP_INFO(logger_, "Loaded %zu class names from %s", class_names_.size(),
              class_names_path.c_str());
}

std::vector<Detection> Yolo11OpenVinoModel::postProcess(
    float *output_data, const ov::Shape &output_shape, float conf_threshold,
    float nms_threshold, float scale_x, float scale_y) {
